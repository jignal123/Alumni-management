  ...
  <div class="container">
    ...
    <form action="" method="post" id="myform" class="row mt-1 p-2">
      ...
      <div class="col-md-4">
        ...
        <div class="input-field">
          <select name="city" id="city" class="form-select mt-2">
            <option value="">Leave it blank if you don't want to change it</option>
          </select>
          <i class="fa" aria-hidden="true"></i>
          <span class="d-flex">
          </span>
      ...
      <div class="col-md-4 image">
        <a href=".<?=$_SESSION['simage'];?>">
          <img src=".<?= $_SESSION['simage']; ?>" alt="userimg" style="width: 70px; height:70px;">
        </a>
        <input type="hidden" name="hidden_img" value="<?=$_SESSION["simage"]?>">
      </div>
      <div class="col-md-12" align="center">
        <input type="hidden" name="user_id" id="user_id" value="<?=$_SESSION["student_id"]?>">
        <input type="hidden" name="hidden_city" value="<?= $_SESSION['scity'] ?>">
        <input type="submit" value="Update" class="activel" id="sub">
        <a href="changepassword.php" class="ps-2">Change Password?</a>
      </div>
    </form>
  </div>
</div>
...
<script>
  $(document).ready(function () {
    $("#fnm").val('<?= $_SESSION['sname'] ?>');
    $("#lnm").val('<?= $_SESSION['ssurname'] ?>');
    $("#gender").val('<?= $_SESSION['sgender'] ?>');
    $("#birth").val('<?= $_SESSION['sdob'] ?>');
    $("#state").val('<?= $_SESSION['sstate'] ?>');
    $("#city").val('<?= $_SESSION['scity'] ?>');
    $("#address").val('<?= trim($_SESSION['saddress']) ?>');
    $("#sprofession").val('<?= $_SESSION['profession'] ?>');
    $("#semail").val('<?= $_SESSION["alumni"] ?>');
    $("#phone").val('<?= $_SESSION['sphone'] ?>');
  });
</script>
